...
exports.getExperimentalFacilities = async (req, res) => {
    try{
        ...
        const ExperimentalFacilitiesData = await ExperimentalFacilities.find({grp : grp}).sort("priority_number");
        return res.status(200).json({status:"Success", data: ExperimentalFacilitiesData});
    }
    catch(err){
        console.log(err);
        return res
...
exports.postExperimentalFacility = async (req, res) => {
    try{
        ...
        const newExperimentalFacilitiesData = new ExperimentalFacilites({experimentalFacilitiesTitle, experimentalFacilitiesImage});
        const ExperimentalFacilitiesData = await newExperimentalFacilitiesData.save();
        return res.status(200).json({ status: "Success", data: ExperimentalFacilitiesData });
    }
    catch(err){
        console.log(err);
...
exports.editExperimentalFacility = async (req, res) => {
    try{
        ...
        const UpdatedExperimentalFacilityData = await ExperimentalFacilities.findByIdAndUpdate(id, data);
        if(req.file){
            fs.unlinkSync(`${__dirname}/../../uploads/grp/${ExperimentalFacilitesData.experimentalFacilitiesImage}`);
        }
        return res.status(200).json({ status: "Success", data: UpdatedExperimentalFacilityData });
    }
    catch(err){
        console.log(err);
        return res
...
exports.deleteExperimentalFacility = async (req, res) => {
    try{
        ...
        const DeletedExperimentalFacilityData = await ExperimentalFacilities.findByIdAndDelete(id);
        fs.unlinkSync(`${__dirname}/../../uploads/grp/${ExperimentalFacility.experimentalFacilitiesImage}`);
        return res.status(200).json({ status: "Success", data: DeletedExperimentalFacilityData });
    }
    catch(err){
        console.log(err);
        return res
