...
exports.getAlumniProfile = async (req, res) => {
    try{
        ...
        const AlumniProfileData = await AlumniProfile.find({grp : grp}).sort("priority_number");
...
exports.postAlumni = async (req, res) => {
    try{
        ...
        const { grp } = req.params;
        const {alumniName, designation, yearOfCompletion, nameOfInstitution, email, phone} = req.body;
        const imagePath = req.file.filename;
        const newAlumniProfileData = new AlumniProfile({alumniName, designation, yearOfCompletion, nameOfInstitution, email, phone, imagePath});
...
exports.editAlumni = async (req, res) => {
    try{
        ...
        const {alumniName, designation, yearOfCompletion, nameOfInstitution, email, phone} = req.body;
        let imagePath;
        let data = {alumniName, designation, yearOfCompletion, nameOfInstitution, email, phone, grp};
        if(req.file){
            imagePath = req.file.filename;
            data =  {alumniName, designation, yearOfCompletion, nameOfInstitution, email, phone, grp};
        }
        const UpdatedAlumniProfileData = await GroupMembers.findByIdAndUpdate(id, data);
        if(req.file){
            fs.unlinkSync(`${__dirname}/../../uploads/grp/${AlumniProfileData.imagePath}`);
        }
        return res.status(200).json({ status: "Success", data: UpdatedAlumniProfileData });
    }
    catch(err){
        console.log(err);
        return res
