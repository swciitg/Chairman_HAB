...
exports.getGroupMembers = async (req, res) => {
    try{
        ...
        const GroupMembersData = await GroupMembers.find({grp : grp}).sort("priority_number");
...
exports.postMember = async (req, res) => {
    try{
        ...
        const { grp } = req.params;
        const {memberName, email, phone, designation, priority_number, googleScholar, researchInterest} = req.body;
        const imagePath = req.file.filename;
        const newGroupMembersData = new GroupMembers({memberName, email, phone, designation, priority_number, googleScholar,researchInterest,imagePath});
...
exports.editMember = async (req, res) => {
    try{
        ...
        const {memberName, email, phone, designation, priority_number, googleScholar,researchInterest} = req.body;
        let imagePath;
        let data = {memberName, email, phone, designation, priority_number, googleScholar,researchInterest,grp};
        if(req.file){
            imagePath = req.file.filename;
            data =  {memberName, email, phone, designation, priority_number, googleScholar,researchInterest,grp};
        }
        const UpdatedMemberData = await GroupMembers.findByIdAndUpdate(id, data);
        if(req.file){
            fs.unlinkSync(`${__dirname}/../../uploads/grp/${GroupMembersData.imagePath}`);
        }
